...
var write = function(affixes) {
  var args = Array.prototype.slice.call(arguments, 1);
  var filePath = generateName(affixes, 'f-');
  var maybeCallback = args[args.length - 1];
  var callbackWithDelete = function (err) {
    if (err) {
      console.error('Temp Error: ', err.toString());
    }
    deleteFileOnExit(filePath);
  };

  if (typeof maybeCallback === 'function') {
    callbackWithDelete = function (err) {
      deleteFileOnExit(filePath);
      maybeCallback(err, filePath);
    };
    args[args.length - 1] = callbackWithDelete;
  } else {
    args.push(callbackWithDelete);
  }

  args.unshift(filePath);

  fs.writeFile.apply(undefined, args);
}
